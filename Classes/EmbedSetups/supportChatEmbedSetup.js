...
module.exports.setup = async (bot) => {
  let channel = bot.channels.cache.get(
    config.support_settings.supportChannelId
  );
  if (!channel) return;
  let message = channel.messages.cache.get(
    config.support_settings.supportMessageId
  );
  if (!message) return;
  ...
  if (
    ...
    message.embeds[0].description !== supportChat.description
  ) {
