const { emailConfig, securityConfig } = require("../Utils/environmentUtils");
const smtp_password = emailConfig.smtpPassword;
      ...
      if (msg.channel.name === `register-${msg.author.id}`) {
        ...
        switch (registration.step) {
          case "sendEmail":
            botLogs(
              bot,
              `Ο χρήστης <@${msg.author.id}> έστειλε το εξής mail για εγγραφή: ${msg.content}`
            );
            ...
            if (tester.test(msg.content)) {
              ...
              client.send(
                ...
                async (err, message) => {
                  ...
                  channel.send(
                    'Τέλεια! Τσέκαρε τα εισερχόμενά σου για ένα μήνυμα με θέμα *"Εγγραφή στο Steki"*'
                  );
                  setTimeout(
                    () =>
                      channel.send(
