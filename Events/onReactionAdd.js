...
const restartRegistration = async (channel, user) => {
    ...
    await db.collection("activeRegistrations").deleteOne({_id: user._id})
    ...
    setTimeout(()=>channel.send("Αν κολλήσεις σε κάποιο απο τα βήματα (βάλεις λάθος ακαδημαϊκό email, δεν λαμβάνεις κωδικό κλπ.) μπορείς να πατήσεις το :arrows_counterclockwise: εδώ κάτω και θα ξανα-αρχίσουμε τη διαδικασία από την αρχή.\nΤο email σου θα αποθηκευτεί σε κρυπτογραφημένη βάση δεδομένων μας, μόνο για να αποτρέψουμε κάποιον άλλο χρήστη από το να το ξαναχρησιμοποιήσει. Χάρη στη μαγεία της :sparkles:πληροφορικής:sparkles: (hashing), δεν θα μπορούμε να διαβάσουμε το email σου απο το αρχείο μας, μόνο να βεβαιώσουμε ότι υπάρχει/δεν υπάρχει εκεί.").then(message=>message.react("🔄")),1000)
