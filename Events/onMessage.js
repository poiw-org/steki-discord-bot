        ...
        bot.on('message', async (msg, user) => {
            ...
            if(msg.channel.name === `register-${msg.author.id}`){
                ...
                switch (registration.step){
                    case "sendEmail":
                                if(tester.test(msg.content) || csdtester.test(msg.content)) {
                                    ...
                                    async function (err, message) {
                                        ...
                                        if(tester.test(registration.email)) channel.send("Τέλεια! Τσέκαρε τα εισερχόμενά σου (https://webmail.edu.hmu.gr) για ένα μήνυμα με θέμα *\"Εγγραφή στο Steki\"*");
                                        else if (csdtester.test(registration.email)) channel.send("Το email που έστειλες αντιστοιχεί σε εξωτερικό τμήμα. Γι' αυτό, θα γραφτείς ως Verified Outsider. Τσέκαρε το ακαδημαϊκό σου email για ένα μήνυμα με θέμα *\"Εγγραφή στο Steki\"*.");
                                        else {
                                            botLogs(bot, `Κατά την εγγραφή του <@${msg.author.id}> σε δεύτερο έλεγχο, φάνηκε το mail να μη πληροί τις προϋποθέσεις εγγραφής. Please check.`)
                                        }
                    ...
                    case "verifyEmail":
                        if(registration.encryptedVerificationCode && sha256().update(msg.content.trim()+salt).digest('hex') === registration.encryptedVerificationCode){
                            ...
                            if(tester.test(registration.email)) member.roles.add("886993717725102103")
                            else if (csdtester.test(registration.email)) member.roles.add("993612908074381352")
                            else {
                                botLogs(bot, `Κατά την εγγραφή του <@${msg.author.id}> σε δεύτερο έλεγχο, φάνηκε το mail να μη πληροί τις προϋποθέσεις εγγραφής. Please check.`)
                                channel.send("Κάτι πήγε στραβά. Δώσε λίγη ώρα για την Ομάδα Διαχείρισης να το ελέγξει...")
                                return;
                            }
