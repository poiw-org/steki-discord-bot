...
const restartRegistration = async (channel, user) => {
  ...
  setTimeout(
    () =>
      channel.send(
        `**Για να συνεχίσουμε, στείλε μου εδώ το ακαδημαϊκό σου email. Θα πρέπει να είναι της μορφής *thXXXXXX@edu.hmu.gr* (αυτό που σου έδωσε η γραμματεία κατά την εγγραφή)**.`
      ),
    2000
  );
};
...
module.exports = {
  ...
  execute: async (bot) => {
    bot.on("messageReactionAdd", async (reaction, user) => {
      ...
      if (
        reaction.message.channel.name === `register-${user.id}` &&
        emoji === "🔄"
      )
        await restartRegistration(reaction.message.channel, user);
